...
class ReportGenerator:
    ...
    def generate(
        ...
        if output_format == "html":
            ...
            return self._generate_sarif(results, target_url, ai_model, output_path)
        else:
    ...
    def _generate_sarif(
        ...
        for result in results:
            ...
            rule_id = f"ANI-{result.category.upper()}-{result.test_id}"
            level = level_map.get(result.severity, "note")

            if rule_id not in rules:
                rules[rule_id] = {
                    "id": rule_id,
                    "name": f"{result.category}/{result.test_id}",
                    "shortDescription": {"text": result.test_name},
                    "fullDescription": {"text": result.test_name},
            ...
            evidence_text = (result.evidence[0] if result.evidence else result.test_name)[:200]
            properties = {}
            if result.metadata.get("regression"):
                properties["regression"] = True
            if result.metadata.get("fixed"):
                properties["fixed"] = True

            sarif_results.append({
                "ruleId": rule_id,
                "level": level,
                "message": {"text": f"{result.test_name}: {evidence_text}"},
                "locations": [{
                    "physicalLocation": {
                        "artifactLocation": {"uri": target_url},
                        "region": {"snippet": {"text": (result.payload or "")[:200]}},
                    },
                }],
            ...
            "runs": [{
                "tool": {
                    "driver": {
                        "name": "ANI",
                        "version": "1.0.0",
                        "informationUri": "https://github.com/local/ani",
        ...
        if not output_path:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            output_path = self.config.reports_dir / f"ani_report_{timestamp}.sarif"
    ...
    def _calculate_stats(self, results: List[AttackResult]) -> Dict[str, Any]:
        ...
        total_tests = max(1, len(results))
        sum_weight = sum(severity_weights.get(Severity(name), 0) * count for name, count in stats["by_severity"].items())
        stats["risk_score"] = min((sum_weight / (total_tests * max_weight)) * 100, 100)
